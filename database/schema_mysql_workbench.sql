...
BEGIN
    ...
    -- Retornar cantidad de sesiones eliminadas
...
SELECT 'PLANES DE SUSCRIPCIÓN:' as STATUS;
SELECT * FROM subscription_plans;

SELECT 'USUARIOS CREADOS:' as STATUS;
SELECT id, email, role, subscription_plan_id, is_verified FROM users;

SELECT 'VISTA CREADA:' as STATUS;
...
║  Email:    admin@plataforma.com                              ║
║  Password: Admin@123                                          ║
║  Plan:     Premium (5 sesiones concurrentes)                  ║
║                                                               ║
║  USUARIO ESTUDIANTE:                                          ║
║  Email:    student@test.com                                  ║
║  Password: Student@123                                        ║
║  Plan:     Basic (1 sesión concurrente)                       ║
║                                                               ║
╚═══════════════════════════════════════════════════════════════╝

NOTA IMPORTANTE:
Los hashes de contraseñas fueron generados con PASSWORD_BCRYPT
y son válidos para usar con password_verify() en PHP.

Para cambiar las contraseñas en producción, ejecuta:
UPDATE users SET password_hash = '$2y$12$NUEVO_HASH' WHERE email = 'usuario@email.com';
